  ...
  return <header className={`site-nav ${scrolled?'is-scrolled':''}`}>
    <div className="site-container flex h-20 items-center justify-between">
      ...
      <nav className="hidden items-center gap-8 md:flex" aria-label="Main navigation">
        ...
        <a className="wa-button" href="https://wa.me/27761324527" target="_blank" rel="noreferrer"><MessageCircle size={18}/> WhatsApp Us</a>
      </nav>
    ...
    {open&&<div className="mobile-menu md:hidden">{links.map(([label,to])=><NavLink onClick={()=>setOpen(false)} key={to} to={to}>{label}</NavLink>)}<a className="wa-button" href="https://wa.me/27761324527" target="_blank" rel="noopener noreferrer">WhatsApp Us</a></div>}
  </header>
}